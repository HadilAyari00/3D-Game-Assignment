...
async function startGame() {
  ...
  scene = await createScene(); // Use 'await' to ensure the scene is loaded
...
function handleTankActions(tank) {
  tank.move();
...
async function createScene() {
  ...
  let tank = await createTank(scene); // Use 'await' to wait for the tank to be loaded.
  console.log(tank);

  // Set the rotation speed for the tank
  // Call the tank.move() function in the main game loop
  ...
  let invisibleTarget = createInvisibleTarget(scene);
  invisibleTarget.position.y = 1; // Set the height of the invisibleTarget

  let followCamera = createFollowCamera(scene, tank, invisibleTarget);
...
function createGround(scene) {
  ...
  function onGroundCreated() {
    ...
    groundMaterial.diffuseTexture = new BABYLON.Texture("images/floor.png");
...
function createFollowCamera(scene, target, invisibleTarget) {
  ...
  if (camera) {
    ...
    camera.position = new BABYLON.Vector3(0, 60, 0);

    scene.registerBeforeRender(() => {
      invisibleTarget.position.x = target.position.x;
      invisibleTarget.position.z = target.position.z;
      invisibleTarget.rotation.y = target.rotation.y;
    });
...
function createInvisibleTarget(scene) {
  let target = new BABYLON.Mesh("invisibleTarget", scene);
...
function createTank(scene) {
  return new Promise((resolve) => {
    ...
    BABYLON.SceneLoader.ImportMesh(
      ...
      scene,
      function (newMeshes) {
        // Set the position, rotation and scaling of the model
        tank = newMeshes[0];
        console.log(tank);
        tank.position = new BABYLON.Vector3(0, 0.6, 0);
        tank.rotation = new BABYLON.Vector3(0, 0, 0);
        tank.scaling = new BABYLON.Vector3(25, 25, 25);
        ...
        tank.fireCannonBalls = function () {
          ...
          cannonball.position = new BABYLON.Vector3(pos.x, pos.y + 10, pos.z);
          ...
          cannonball.position.addInPlace(
            this.frontVector.multiplyByFloats(5, 5, 5)
          );
        ...
        tank.fireLasers = function () {
          ...
          let origin = new BABYLON.Vector3(
            tank.position.x,
            tank.position.y + 10, // adjust the y component here
            tank.position.z
          );
